...
37. Versioned kits with safe inventory snapshots, cooldowns, one-time policy, per-kit dynamic permissions, atomic capacity checks, optional bounded overflow dropping, administrative validation, metadata export, usage reset, and load-time rejection of orphan or over-limit use history.
...
18. Password-like roots, private-message roots, moderation reasons, nested command wrappers, data-command arguments, unknown roots, and every IP moderation alias are redacted before command spy, recent history, file logging, search, export, or audit projection.
