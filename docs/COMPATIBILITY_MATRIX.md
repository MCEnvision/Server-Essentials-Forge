...
Phase 001 storage hardening applies bounded no follow reads to content addressed Fancy Tags objects and validates audit, module, history, backup, write, and recovery directory components without following symbolic links. Audit appends use a securely held directory handle and fail closed when hard link counts cannot be verified. Standard Java NIO cannot provide portable opened descriptor identity, and the default Windows provider lacks `SecureDirectoryStream`, so those runtime environments remain blocked pending an approved provider strategy. Unsafe parents, active audit links, non regular targets, and oversized existing objects fail closed before external state is read or written.
