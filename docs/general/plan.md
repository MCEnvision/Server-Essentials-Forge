...
| SRC-611 | owner_request | Trident for all client instance testing, existing Microsoft authentication and removal of mandatory game muting | research/sources/trident-testing.md, observed 2026-09-26 | Resolve DEC-019, preserve laptop-only execution and personal state, and leave authentication/runtime claims pending execution verification. |
| SRC-612 | reference | Trident instance lifecycle, home/context resolution, scoped settings, account status and deferred deletion | https://github.com/TridentCore/Trident.Net/blob/27e5d858c7959e6e7adc4f127b6a58697caa6cf1/docs/CLI.md | Pin documented command semantics and their limitations. Installed help, actual client loading, input, rendering and disk cleanup still need execution evidence. |
...
### DEC-019 — Trident client testing

**Status:** RESOLVED
**Selected choice:** Use Trident CLI on the verified laptop for all new Minecraft client instance testing, with the existing authenticated Trident home and a uniquely identified task-owned instance. Never run Trident on node-1 or fall back to Prism. Preserve personal instances, account stores, global configuration, shared caches and audio settings. The owner reports an existing valid Microsoft sign-in; verify usable authentication through nonsensitive CLI status before execution without reading or copying credentials. Required gameplay, rendering, joined-world and cleanup evidence remains mandatory.
**Rationale:** One explicit instance lifecycle replaces the previous launcher procedure without adding a product dependency or weakening real client verification. Current owner runtime policy removes mandatory muting, audio watchers and playback-stream proof.
**Affected requirements:** SEF-REQ-010, SEF-REQ-018, SEF-REQ-020
**Supersedes:** Earlier launcher selection and mandatory client-audio isolation instructions only.

Later resolved audit choices supersede pending-choice language in early research. The standing host policy is resolved: node-1 is headless only, all Trident commands and actual clients run on the verified laptop, server-only claims require no owner join, and singleplayer requires a named integrated-server bug. EULA acceptance is already authorized and must be configured and read back in each disposable runtime. Exact owned-resource cleanup applies to every check. DEC-019 changes verification tooling only; SEF remains server-only and does not ship or require a launcher.

## 10. External Prerequisites

| ID | Prerequisite | Affected requirements | Availability | Authorization | Required external action |
|---|---|---|---|---|---|
| EXT-001 | Existing authorized node-1 compute and owner laptop desktop with authenticated Trident CLI over private connectivity | SEF-REQ-010, SEF-REQ-018, SEF-REQ-020 | available | authorized | Revalidate laptop identity, installed CLI help, existing home, nonsensitive authentication status, discrete renderer and exact private endpoint before dependent execution. Owner-reported login is not a completed runtime test. |
...
- Pinned independent adapter fixture with proxy/backend/client logs, actual Trident laptop interaction, denied spoof attempt, observed clientresetpacket capability, two-way in-session transfer and a diagnostic missing-capability control; downloaded bytes or manual reconnect alone fail acceptance.
...
- Phase 006 delivers the complete feature and documentation; Phase 015 revalidates it with the final paired artifacts, all later transfer/RTP paths and the complete presentation inventory. All owned fixture, instance and process cleanup must pass.
...
| Rendering, click input, chat actions, tab/entity visibility, actual client reconnect/switch | Verified Linux laptop | Existing authenticated Trident CLI, active Hyprland, actual discrete NVIDIA renderer and authorized connection/input controls | Explicit task-owned Trident instance key and resolved runtime path in the verified existing home; matching artifacts and pinned client profile | CLI/home/instance binding, exact window/PID/renderer, client/server join correlation, targeted visual evidence and verified teardown |
...
Never run Trident or Minecraft clients on node-1, including CLI listing/building, offscreen, Xvfb, VNC, X11 forwarding, software rendering or an integrated GPU workaround. Every Trident command executes on the verified laptop, including when issued through the existing private connection. Bind the exact owned `hyprctl clients -j` window address, class, title and PID to the launched candidate and confirm its actual discrete renderer. Revalidate PID and address before input or capture. Never use `hyprctl repl` or bypass the guarded control executable. Missing host, instance, renderer, window, authentication or connection evidence leaves the affected client gate open; continue independent headless work. Leave audio unchanged, do not create mute watchers, and do not require a playback stream or mute proof.

#### Trident instance lifecycle

The CLI reference is SRC-612. Compare the pinned documentation with the installed binary's help before using version-specific flags. Record the executable/version, laptop host and desktop, existing Trident home, owned instance key and resolved profile/runtime paths, Minecraft/Forge/Java versions, dependency and candidate hashes, private endpoint and teardown targets. Trident is a verification tool, never a distributed SEF dependency. Its installation and account are owner-reported available until the execution preflight verifies them.

Resolve the existing home using documented precedence: `TRIDENT_HOME`, the nearest ancestor `.trident`, the first absolute path line in `~/.trident.home`, then `~/.trident`. An accidental project-local home is not authorization to initialize another one. Bind calls to the verified existing home and explicit instance key; never redirect to a disposable home or rely on implicit working-directory selection. Use supported account status to verify Microsoft authentication, without reading, copying, printing or exporting credential stores such as `.trident.cli/accounts.json` or `repositories.json`. If the existing session requires device authorization, report that exact owner step; never switch to offline authentication or another launcher.

Inventory instance keys before allocation. Create one uniquely named task-owned instance with the pinned Minecraft and Forge versions using installed-help-confirmed `instance create --identity <owned-key> --name <test-label> --version 1.20.1 --loader net.minecraftforge:47.3.12`, or the verified import equivalent when the fixture requires a pack. Inspect with `instance inspect --instance <owned-key>` and inspect its package inventory separately. Reuse that owned instance across dependent cases when valid. Verify the supported local candidate JAR installation procedure instead of inventing a CLI flag; inspect the resolved runtime before placing any candidate. Pin dependency files, including EXT-010 only for the selected network profile, and prove the running client loaded the intended bytes and capabilities. The standalone control has no reset adapter or required SEF client.

Always scope mutable configuration with `--instance <key>`; unscoped configuration changes global settings and is forbidden for a fixture. Use the instance's `behavior.connect.address` for the verified private endpoint where supported. Use `instance build --instance <key>` and `instance run --instance <key>` with the verified Java 17 launch path and bounded memory, plus an existing verified discrete-GPU wrapper when needed. Build-time Java selection is not proof of launch-time Java. Validate the actual renderer and runtime after launch. Preserve real process handles and bounded waits; JSON mode may emit only a final result during a long build. Nonzero or partial-success results leave the affected operation incomplete.

Trident provisions and launches instances; its documented CLI does not establish gameplay input or screenshot capabilities. Use only authorized laptop controls for natural input and targeted captures, serialize shared-desktop interaction, and verify the intended window before each action. Confirm server readiness, then auto-connect through verified version-specific quick-connect behavior or authorized desktop controls without editing the personal server list. If a necessary capability is unavailable, report it precisely; do not expose ports, weaken authentication or replace real client evidence with console assertions.

Register teardown before creation, build or launch. After the final consumer, stop only owned client/server processes and verify exit, retain required sanitized evidence, then use `instance delete --instance <key> --yes` after rechecking ownership and paths. The documented command marks deletion and removes the instance from the active manager; it is not proof that disk state is gone. Verify both instance-list absence and the exact owned paths. Remove only proven task-owned residual files under the cleanup policy or report cleanup incomplete. `instance reset` retains profile/import/persisted data and is not teardown. Never remove the Trident home, shared caches, credentials, personal instances or project checkout. Preflight, launch, connection, input and cleanup failures preserve their distinct evidence and narrow rerun requirements.
...
Every verification run registers exact owned paths/processes before starting and teardown on every exit path. Use nested disposable directories under the verified project anchor after exact parent Git/build/index/package exclusions, never sibling clones or copied projects. Laptop client instances are the explicit exception: allocate them only inside the verified existing Trident home under the lifecycle above, never under an invented laptop project or launcher root. Identify preexisting content in reused directories. Keep only minimal sanitized evidence at `docs/verification/phase-NNN/` and test procedures under `docs/test/`; retain candidate artifacts only through final consumers or as requested deliverables. Stop exact owned processes, verify exit, release locks/tickets/watchers/routes, and remove only verified disposable files without following symlinks. Preserve source, tracked fixtures, shared caches, personal worlds, unrelated worktrees and historical Git. Verify absence on every host. Cleanup failure keeps the gate incomplete even when tests pass. A read-only check that creates nothing records that fact.
...
6. Keep only required evidence under the phase verification directory and the support packet described in `docs/troubleshooting/diagnostics.md`. Stop exact owned clients, servers/proxy/database fixtures, verify processes have exited and owned instances are removed, remove their verified disposable outputs after the final consumer and confirm cleanup on both hosts. Preserve reusable diagnostics and documentation. Record exact leftovers separately if cleanup fails.
...
- Diagnostics, support commands, budgets, permissions, redaction and artifact-tested runbooks are complete. Required real client claims have actual Trident laptop evidence; no server-only surrogate closes input, rendering or synchronization.
- Every client used the verified laptop Trident instance lifecycle with unchanged audio settings; every test/audit cleanup is verified on all used hosts. Exact unresolved leftovers or unavailable prerequisites keep completion open.
...
Locked owner decisions: DEC-001 through DEC-019, plus standing host, Trident client, EULA, cleanup and sequential integration policies.
...
Required evidence gates: per-requirement ACs, phase exits, real path and residual Trident client proof, dual SQL engines, failure/recovery/conservation matrices, measured budgets, private additive review when available, required checks, resulting product commits, signatures and per-host cleanup.
```
