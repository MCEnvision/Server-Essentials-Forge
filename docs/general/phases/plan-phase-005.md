...
5. Stop only owned clients, proxy and backend processes within a 30 second shutdown bound and verify their exit and owned instance removal. Remove exact disposable runtimes, SQLite/WAL/SHM files, backups, logs, downloads, traces and temporary databases after their final consumer; verify absence on both hosts while preserving required evidence, shared caches, personal worlds and unrelated paths.
