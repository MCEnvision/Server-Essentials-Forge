...
6. `P003-TASK-006` executes server-first command, persistence, and teleport fixtures, then the minimal residual silent laptop claim. Trace: SEF-PHASE-003.
...
**Client verification:** One residual laptop check verifies a real player uses the retained command input and receives readable localized rich vanilla feedback. It does not substitute for dedicated-server safety proof. If desktop, renderer, window-to-stream identity, or mute verification is absent, stop the owned client and leave only this visual/input gate open.  
**Client audio isolation:** Before launch set the isolated pinned-version master output to zero. After launch bind the exact owned Hyprland window address, class, title, and PID from `hyprctl clients -j` to the launched process tree, correlate only that tree to its PipeWire or PulseAudio application stream, mute it with `wpctl` or `pactl`, and read back that exact stream as muted before assertions. Immediately identify, mute, and verify any replacement stream after reload, device change, reconnect, or resource recreation. If identity or mute proof fails, stop the owned client and leave the gate open. Teardown stops the client and watcher and removes temporary audio state after confirming the stream is gone.  
...
6. Retain only the sanitized candidate identity, test result, decisive diagnostics excerpt, documentation proof, and required targeted client artifact under the Phase 003 verification destination. Stop exact owned server, client, watcher, and fixture processes; verify their exit and playback-stream disappearance; remove exact disposable runtime, world, logs, crash reports, watcher state, temporary audio state, reports, and incidental bytecode after their last consumer.
...
| P003-TASK-006 | Client adapter snapshot is not a substitute for this row. | Match laptop candidate to node-1 server artifact/config/world generation. | One targeted laptop native input and rich-feedback receipt after server readiness and joined-world proof. | Missing desktop/discrete renderer/window-to-stream identity/mute leaves only this gate unverified and stops the owned client. | Linux laptop for client only, node-1 dedicated private server. Isolated client uses matching candidate and verified per-application mute. | Window/PID/renderer/muted-stream proof, joined-world correlation, targeted evidence, both-host cleanup receipt. |
...
| P003-RISK-002, P003-TASK-005 and P003-TASK-006 | Default-off bounded capture, strict target resolution, audio isolation, and registered teardown. | Capture cap/status, redaction scan, watcher/stream/process absence checks. | Disable capture, stop owned client/watcher/server, remove only owned scratch, and report leftovers. | Diagnostic, client, and cleanup receipts. | Rerun control, real-path, and per-host cleanup procedure. |
...
- Resource register and cleanup receipts: exact nested disposable runtime/world/log/report paths, `eula=true` readback when a server ran, owned process exit, client watcher/stream removal when used, retained sanitized evidence destinations, and verified absence of disposable outputs on each used host.
