...
- A dedicated no-GUI Forge fixture proves then-existing command dispatch uses the policy/result pipeline. A matching laptop fixture supplies the narrow native rich-chat click/hover rendering receipt only after all silent-client gates pass.
...
**Client audio isolation:** Before launch, set that isolated pinned test instance master output to zero, resolve the exact owned Hyprland address/class/title/PID with `hyprctl clients -j`, correlate only that owned process tree to its PipeWire or PulseAudio playback stream, mute the verified per-application stream with `wpctl` or `pactl`, read back muted status before acceptance, reapply after resource reload/device change/reconnect/stream recreation, then stop the watcher and client, remove temporary audio state, and verify the owned stream disappeared at teardown.
...
- If the residual client gate is attempted, verified laptop project/runtime anchor, matching candidate/profile, discrete renderer, prelaunch isolated master output zero, exact Hyprland address/class/title/PID, owned PipeWire/Pulse stream correlation and muted readback, replacement-stream handling, private endpoint/readiness, both-side joined-world proof, targeted visual evidence, watcher teardown, stream disappearance, and exact cleanup receipt. If any part is missing, record the open gate rather than substituting server evidence.
