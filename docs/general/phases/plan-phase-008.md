...
- SEF-REQ-026 pre-sink credential and sensitive-field redaction, manual normal retention policy, emergency local-only rotation, honest gaps, health, and recovery foundations.
...
- SEF-REQ-025 textual inspection, queries, export, API exposure, and central/manual purge execution belong to Phase 010. This phase records policy and prevents automatic SQL deletion; it does not add a query or purge authority.
...
- Use no automatic normal purge and no SQL auto-purge. Emergency capacity rotation considers only the oldest verified closed SEF-owned segment in the canonical spool after gap metadata is durable where possible. Trace: SEF-PHASE-008.
...
**Client audio isolation:** No Minecraft client is launched for this phase. If a later unexpected client claim is proposed, discover the laptop anchor and isolated runtime, verify desktop and discrete renderer, set prelaunch master output to zero, bind the exact Hyprland window and PID to only its PipeWire or Pulse stream, mute and read back that stream, recheck replacement streams, then stop and clean it; otherwise leave that client gate open.  
