...
5. Run the documented negative, restart, recovery or refusal path. For audit, use actual MySQL then actual MariaDB evidence for backup, migration, manual purge, emergency rotation, loss ledger and alerts. For lifecycle verify all four families and UUID visits. For RTP verify local and proxy durable fairness, strict distance budgets, leases, recovery and unavailable paths. If the owned application stream changes, rebind and verify mute before continuing.
