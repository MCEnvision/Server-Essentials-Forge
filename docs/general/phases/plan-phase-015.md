...
|---|---|---|---|---|
| OBSERVED | baseline | Forge baseline is 1.20.1, Forge 47.3.12, Java 17; Velocity candidate is Java 25. Ambassador, ProxyCompatibleForge and the pinned client reset mod require real compatibility proof. | SRC-002, SRC-104 through SRC-109, SRC-118, SRC-119 | Any artifact, client capability, dependency, common/protocol digest, loader or pack change invalidates affected rows. |
...
| OBSERVED | host boundary | `node-1` is headless only. Real chat, input, rendering, tab and entity visibility need owned Trident laptop clients. | EXT-001, SRC-004 | Desktop, discrete renderer, exact client, Trident instance to window/PID binding or private endpoint change invalidates client evidence. |
...
**Owner:** Integrated verification  
**Dependencies:** SEF-PHASE-014, EXT-001, EXT-003, EXT-004, EXT-005, EXT-006, EXT-007, EXT-010
...
- Use the fixed one-proxy three-backend topology, the actual MySQL and MariaDB fixtures, and isolated Trident laptop clients only for residual input, presentation, rendering, tab and entity-visibility claims. Trace: SEF-PHASE-015.

**Required evidence**

- A complete row-level `SEF-AC-001` through `SEF-AC-036` matrix, independent source/sink/cursor/item-entity/world oracles, both actual SQL engines, exact candidate and host bindings, measured workload results, fault/recovery evidence and per-host cleanup receipts. Include a separate clean one-server Forge installation without Velocity, Ambassador, ProxyCompatibleForge or the client reset mod, with loader metadata inspection and actual local command, storage, homes, lifecycle, audit and RTP receipts.
...
- Both actual SQL engines pass all capture, query, migration, restoration, undo, privacy and capacity rows; all client claims have Trident laptop evidence; cleanup receipts exist for every host and fixture.
- The one-server Forge profile passes with no external proxy adapter installed or required by loader metadata. Network-profile acceptance remains separate and still proves the pinned adapter and reset-capable client matrix.
...
|---|---|---|---|---|---|---|
| P015-TASK-001 | SEF-REQ-020, SEF-REQ-001, SEF-REQ-009 through SEF-REQ-020, SEF-REQ-030, SEF-REQ-036 | Run the separate standalone Forge installation and one-proxy, three-backend topology, presentation and lifecycle closure. | P015-TASK-003, SEF-IF-004 through SEF-IF-008, SEF-IF-015 | Standalone startup and local feature receipt plus correlated topology, client, presentation and lifecycle evidence. | Proxy, Forge dispatcher/chat, presence, moderation, visibility, homes, transfer, renderer, lifecycle. | Matrix below, with actual server and client oracles; inspect Forge loader metadata and start the local profile with no proxy or reset adapter. |
...
7. Stop only owned clients, servers, proxy and database fixtures; each shutdown has a 30 second bound. Verify process exit and owned instance removal, remove exact disposable paths without symlink traversal and record cleanup independently from pass/fail.
...
| SEF-REQ-001, SEF-REQ-020, P015-TASK-001 standalone profile | Forge loader metadata and dependency scan | Clean one-server installation with network mode disabled and no Velocity, Ambassador, ProxyCompatibleForge or client reset mod | Local commands, local homes, lifecycle, audit capture and lookup, and local RTP operate without external proxy adapters or companion prompt | Missing external adapter must not fail local startup or local features | node-1 dedicated Forge server and isolated SQL fixture; Trident laptop only for actual client input and rendering claims | Startup, loader scan, local feature results, absent-prompt check and cleanup receipt. |
| SEF-REQ-009, SEF-REQ-011, SEF-REQ-020, P015-TASK-001 companion admission | Manifest-backed install guidance and permission checks | Ambassador network configured with missing then unreachable SEF Velocity companion | Backend console and authorized admin-only status show matching companion installation or connection guidance and network-unavailable state | Never claim absence from a timeout, prompt players, auto-download, expose secrets or silently fall back to local authority | node-1 dedicated Forge and isolated Velocity fixture, no client required | Operator transcript, distinct failure reasons, network refusal and cleanup receipt. |
| SEF-AC-010 through SEF-AC-018 | Codec/revision state tests | Empty bridge, profile registry, authority | Same-pack and compatible-different in-session switching with pinned reset-capable client; signed chat; ban/mute; vanish; homes; consent | Missing client reset capability, unknown or incompatible pack, forged/replay/stale, direct identity, offline, disconnect/reconnect mistaken for switch | one proxy plus three node-1 backends; Trident laptop roles | Correlated proxy/backend/client matrix and unchanged-source refusal receipts. |
...
For topology, perform same-pack and direction-specific compatible-different continuous-session transfers with the pinned external client reset adapter; remove it in a fresh isolated profile to prove clear pre-move refusal and unchanged source state, then test unknown and incompatible refusal. Validate installed pair and client adapter identity and reject profile/adapter/registry evidence drift. Cover zero-player backend bridge, forged/replayed/stale/oversize frames, key rotation, authority loss/reconnect, offline target, concurrent mutation, ban/login rejection, signed muted/unmuted public and private/alias routes, ordinary and authorized vanish observers, observer permission revoke, tab/entity/selector/suggestion surfaces, and rapid reconnect versus stale session.
...
- Per-host cleanup register showing exact owned server/proxy/database/client processes stopped, owned instances removed, retained sanitized evidence readable and disposable paths absent. Cleanup failure remains an open gate.
