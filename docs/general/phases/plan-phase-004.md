...
|---|---|---|---|---|
| OBSERVED | Proxy compatibility | Velocity 4.2.0 build 30, Ambassador 1.4.5, and ProxyCompatibleForge 1.3.1 are pinned inputs, while their combined runtime compatibility remains unverified. | SRC-101, SRC-102, SRC-104, SRC-106, SRC-107, SRC-109; FIND-101 | Recheck every artifact identity, advisory status, license notice, and Phase 000 runtime receipt before use. |
...
- The Phase 000 feasibility matrix still records a passed matching pinned matrix or an explicit blocker. A blocker stops this phase before implementation rather than inviting a custom handshake fallback.
...
| `P004-TASK-003` | SEF-REQ-009, SEF-REQ-011 | Prove reconnect and bounded credential rotation recovery, enforce documentation and support controls, and integrate sequentially. Merge Forge first, verify its resulting `forge-1.20.1` commit and tag, retest proxy against that exact approved common digest, then merge and tag Velocity. The task waits for both resulting branch checks and tags before closure. | P004-TASK-001, P004-TASK-002, EXT-002, SEF-IF-005 | Reconnect and rotation receipts, operator runbook, documentation updates, ordered branch PR evidence, resulting branch receipts, signed tags, and cleanup ledger | Planned bridge lifecycle, peer-key configuration, operator documentation, README and documentation index | Close one channel and reconnect it with a fresh channel epoch under unchanged process boot; replay its prior frame. Restart one owned process to prove stale boot rejection separately. Rotate only from expected identifier to one successor while no more than two explicit keys overlap, then revoke predecessor and fence its old channels. |
...
**Client audio isolation:** No Minecraft client is launched for this phase. If a later authorized residual fixture requires one, set only the isolated pinned-version instance master output to zero before launch, verify the laptop desktop and discrete renderer, resolve the exact owned Hyprland window address, class, title, and PID with `hyprctl clients -j`, bind that PID's owned process tree to its PipeWire or PulseAudio playback stream, mute only that stream with `wpctl` or `pactl`, read back muted state before assertions, reapply to every replacement stream after reload, device change, reconnect, or recreation, fail closed by stopping the owned client when identity or mute is uncertain, then tear down the client, watcher, and temporary audio state and verify its streams are gone. Never mute a default sink, microphone, personal instance, or unrelated application.  
...
5. Disable capture, read `status` to confirm it stopped, redact the required signal fields, preserve only stated sanitized evidence and requested artifact manifests, stop all owned harnesses and watchers, confirm exit and stream absence, remove exact fixture runtime, temporary audio state if any, logs, traces, downloaded test inputs, bytecode, and scratch reports, and verify every registered owned path is absent. Report leftovers separately and leave the gate open until removed.
