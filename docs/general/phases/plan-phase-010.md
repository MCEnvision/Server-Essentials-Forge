...
- SEF-REQ-029. Versioned CE24.1/catalog/API parity registry, original Forge adapter API, queue versus central-history status, pause/resume/status controls, operator-safe manual purge and explicitly enabled scheduled retention contracts, and bidirectional MySQL/MariaDB migration evidence.
...
- Custom GUI, screen, menu, client protocol, client-side audit storage, arbitrary console execution, unconfigured or capacity-triggered SQL purge, production database access, credential collection, and a public release are outside this phase.
...
- Consumer pause suspends SQL ingestion while retaining local capture, visible backlog and frozen central watermarks. Separately authorized capture pause records its actual missing-history interval. Resume cannot infer unrecorded activity; ingestion resume replays durable events idempotently. Trace: SEF-PHASE-010.
- Manual purge is an operator-only, audited preview/confirm flow: a preview freezes exact selected EventKeys and selection digest, shows resulting intentional gaps, requires a fresh confirmation, and refuses any selection when the Phase 011 restoration-dependency provider is unknown, unavailable, or reports unresolved dependencies. Scheduled retention is a distinct explicit opt-in policy using the same deletion guards, never an implicit replacement for manual defaults. The Retention Policy and Execution subsection defines its activation, configuration, serialization and recovery. Trace: SEF-PHASE-010.
...
- The predicate compiler has fixtures for actor and pseudo-actor, origin, world, radius, box, selection, global scope, UTC and relative time, action and outcome, positive and negative registries, literal text-prefix inclusion and exclusion, confidence and correlation. Near/count/material summaries and permitted location jumps reuse the same authorization predicate and existing canonical typed command routes. Trace: SEF-PHASE-010.
...
6. P010-TASK-006 delivers diagnostics producer coverage, command self-tests, failure fixtures, and the required Trident laptop inspection/click/chat receipt. Trace: SEF-PHASE-010.
...
- Natural Forge interaction evidence showing inspection cancellation without a block mutation, plus a required actual Trident laptop click/hover/rich-chat receipt. Server logs are supporting evidence only.
...
- Inspector, every listed filter, page, detail, hover, summary, bounded export, pending/watermark/gap/uncertainty display, consumer and capture pause/status, manual and optional scheduled retention, extension registration, and dual-engine migration paths pass their required proof without custom GUI or unintended world mutation.
...
|---|---|---|---|---|---|---|
| P010-TASK-001 | SEF-REQ-025, SEF-REQ-029 | Build the versioned CE24.1/catalog/API parity registry and query service. Compile every required typed filter, actor/scope authorization, frozen origin cutoff, stable sort and cursor; apply identical predicate and redaction policy to page, detail, hover, summary and export. Reauthorize every continuation, use literal native-chat values, and make missing/gap/uncertain coverage visible. | Phase 009 actual capture and coverage; SEF-IF-003, SEF-IF-004 and SEF-IF-009; DEC-008 through DEC-014. | SEF-IF-010 lookup/export model, parity rows, filter/index specification, permission matrix, cutoff/cursor policy. | Planned audit investigation service, parity registry, command adapters, native component renderer, SEF-IF-010. | Individually executable fixtures cover actor and pseudo-actor, origin, world, radius, box, selection, global scope, UTC and relative time, action/outcome, positive/negative registries, literal text-prefix inclusion and exclusion, confidence and correlation; summaries and permitted location jumps prove the same predicate and canonical typed routes. Test default 10 rows, valid continuation beyond page 10, rejection above 50, nested sensitive NBT/command sentinel scans, ingest-between-page snapshot, offline-origin query, and executable CE24.1 categories: inspection, lookup/filter/page/summary, queue/status, preview/apply/cancel, rollback/restore/undo, recorded-item give, purge, permissions and typed extension API. Each category has an owner, evidence, or explicit technical platform reason. |
...
| P010-TASK-005 | SEF-REQ-025, SEF-REQ-029 | Add ordinary Forge click inspection and textual controls for lookup, near/count/material summaries, permitted location jumps, status, queue/pending state, scoped pause/resume, manual purge preview/confirm and guarded scheduled retention as specified below. Inspection cancels only its active audited interaction and cannot change a block, container, item or audit event. Consumer pause preserves capture and reports backlog; explicit capture pause records a missing-history interval. Both purge paths freeze exact selection and refuse an unknown, unavailable or unresolved future restoration-dependency provider. Implement policy activation, fenced scheduling, cancellation, checkpoint recovery and config validation before any scheduled deletion. | P010-TASK-001; Phase 009 terminal events; existing action-token policy; future Phase 011 dependency-provider contract and safe synthetic reference fixture only. | Inspector command/interaction contract, operator control grammar, pause coverage records, audited purge preview/confirmation, scheduled-policy activation and bounded retention job contracts. | Planned Forge audit interaction adapter, query controller, audit health/coverage view, SEF-IF-004 and SEF-IF-009. | Natural right/left interaction fixture through real Forge entry point with normal player permission, cancellation oracle and before/after world digest; inactive/denied click preserves normal action; stale token/revoked permission/expired preview refuse; consumer pause/resume preserves captured events and drains backlog without a false gap; explicit capture pause/resume reports its exact missing-history interval; unknown, unavailable or unresolved dependency provider blocks either purge path without consulting a central job table. Run every retention fixture in the Retention Policy and Execution subsection on both SQL engines, including a standalone Forge process with Velocity absent. |
| P010-TASK-006 | SEF-REQ-025, SEF-REQ-029 | Deliver investigation-specific typed diagnostics before dependent acceptance, including query decision, cutoff, page, export, inspector interaction, parity disposition, migration checkpoint and purge decision signals. Register auditable target types, default-off controls and source-unit self-test. Execute the required laptop-only residual inspector/click/hover/native-chat receipt after the headless proof is ready. | P010-TASK-001 through P010-TASK-005; SEF-IF-002; actual candidate and host identity. | Registered diagnostic categories, self-test, sanitized support packet and residual client evidence. | Planned diagnostics producers in investigation, migration and interaction services; docs/troubleshooting/diagnostics.md. | Console enable/status/off permission tests, absent target and output-unavailable test, self-test then disable then re-enable before the real stimulus, 60-second capture limit, and actual Trident laptop click/hover/chat proof. |
...
The parity registry is a local versioned record, not an imported or copied CoreProtect API. Each row contains pinned reference/version, category, requested function, Forge-native equivalent, canonical owner phase/task, implementation status, test evidence, limitations, and N/A reason. The phase-owned public extension surface is original AuditAdapterV1 and SEF-IF-010. Parity convenience operations compose the existing SEF-IF-009 append, SEF-IF-010 query and SEF-IF-003 policy methods; Phase 011 supplies SEF-IF-011 guarded restoration. No convenience wrapper grants additional authority. It validates descriptors and coverage receipts at registration; it does not load unknown plugins, run adapter code on the SQL worker, or accept Bukkit world/player/block types. Consumer pause is an ingestion state with exact start/end, backlog and frozen central watermarks, not an automatic capture gap. Explicit capture pause is separately authorized and records its unobserved scope and interval. Actual spool loss during either pause remains a durable loss boundary. Neither pause silently changes EventKey ordering.
...
### Retention Policy and Execution

P010-TASK-005 implements the following schema under `audit.retention` in the existing main configuration. Phase 008 validates the same schema but does not execute it. No client setting or Velocity process owns this policy.

| Field | Type and default | Validation and meaning |
|---|---|---|
| `mode` | Enum, `MANUAL` | Accept only `MANUAL` or `SCHEDULED`. Missing legacy settings remain manual and cannot activate deletion. |
| `keep_days` | Integer, `180` | Accept 1 through 36500 days. Delete only records strictly older than the run's frozen UTC cutoff. This value alone authorizes nothing. |
| `run_at_utc` | String, `03:00` | Strict 24 hour `HH:mm`, evaluated in UTC once per date. No timezone or daylight saving ambiguity. |
| `origins`, `worlds`, `event_kinds` | Typed lists, empty | Explicit dataset and origin scope is mandatory for activation. World identities include generation; kinds use known descriptors. Empty selectors never mean all. Global scope requires a separately permissioned explicit choice in the activation preview. |
| `batch_rows` | Integer, `512` | Accept 1 through 512. Each transaction also respects the existing 1 MiB batch bound and 15 second deadline. A single oversized candidate is refused visibly, not silently skipped or truncated. |
| `run_budget_seconds` | Integer, `60` | Accept 1 through 300. Stop at a batch boundary, retain durable progress and disclose remaining eligible records. Never extend the budget to catch up. |

Parse and validate a complete immutable candidate snapshot before atomic reload. Reject unknown fields, invalid bounds, ambiguous scope and stale world identities without replacing the last valid snapshot. Changing a policy invalidates its prior activation digest. A syntactically valid `SCHEDULED` setting therefore remains inactive until an authorized operator previews its dataset, selectors, cutoff rule, next UTC run, limits and irreversible consequences and confirms that exact revision through the existing safe action contract. Persist the policy digest, confirming actor and authorization scope separately from config. Recheck that authority before each batch. A valid change to manual mode or an explicit authorized cancellation stops scheduling before the next batch. Invalid reload does not partially apply fields or cancel unrelated state; the command remains available to stop a running policy. No reload silently arms or expands a schedule.

The configured backend owns the scheduler; SQL owns only its dataset lease, monotonic fencing token and retention receipts, not restoration jobs. Standalone Forge uses the same service without Velocity. Multiple backends sharing a dataset contend for one transactional lease. Every manual and scheduled batch verifies the current fence, policy revision and authorization in its deletion transaction. Migration takes the same exclusion guard; lease loss, shutdown, migration, ingest-consumer pause or unavailable dependencies stops further batches. Use a 15 second lease, renewal every 5 seconds and database time. A delayed former holder cannot write with an old token. Each scheduled run has the unique key `(dataset, policyRevision, UTC-date)`, so restart or duplicate delivery cannot launch it twice. Restart does not replay missed dates or automatically resume an interrupted destructive run; an explicit authorized resume may use only its saved selection and fresh dependency checks.

Freeze the run cutoff and exact EventKeys before mutation using bounded durable selection pages, not an unbounded heap list. Persist selection digest, next page and applied counts. Do not select records that remain unacknowledged locally or that cannot be proven independent of active restoration dependencies. Coordinate the dependency provider and new restoration selection admission under the same dataset guard so a job cannot acquire a purged preimage between a check and deletion. An offline or unknown dependency owner blocks affected scope. Phase 010 tests synthetic providers; Phase 011 supplies and proves the real local job provider. Commit each batch's intentional-purge range, deletion and progress atomically. Remove payload blobs only after proving that no remaining event or protected job references them. Retain compact purge receipts outside their own selection; they must not disappear in the job that creates them. Central ingest and replay consult these receipts before insertion so an old retry cannot resurrect intentionally purged EventKeys. Migration and backup preserve those receipts.

Both paths preserve authority databases, restoration job ledgers, RTP state and unrelated tables. SQL purge is never a response to disk pressure. Deletion may not immediately return disk space to the operating system; report that fact and leave table optimization to a separate explicit maintenance procedure. Health output shows configured mode, activation state, owner/fence, last result, next run, deleted and remaining counts, and blocked reason without protected event content. Audit activation, cancellation, refusal and completion. Discord uses only the existing sanitized incident contract.

Required P010-TASK-005 fixtures cover legacy/missing defaults; valid and invalid reload; explicit activation and changed-scope refusal; permission revocation; UTC date and age boundaries; no restart catch-up; competing backend leases and expired holders; manual versus scheduled and migration races; ingestion pause; unknown/offline/unresolved job dependencies; dependency acquisition racing deletion; cutoff stability under concurrent ingestion; purged-event replay without resurrection; shared blobs; row/byte/time budgets; cancellation; crash before and after transaction commit; truthful gaps and disk-space messaging; and successful local scheduling with no proxy installed. Run actual deletion and recovery fixtures on both MySQL and MariaDB. Phase 015 repeats protection with real restoration jobs and the three-backend topology; Phase 016 rehearses the operator activation, cancellation and recovery instructions.

### Functional Parity Closure

P010-TASK-001 expands every referenced feature and API operation into an individual parity row, rather than accepting one broad category checkbox. The required row families below supplement the complete action inventory in Phase 009. They remain mandatory wherever the Minecraft 1.20.1 Forge platform can supply the behavior.

| Required row family | Owner and acceptance |
|---|---|
| Inspector, lookup, near, count, actor/material summary, pages, item hover and permitted location jump | P010-TASK-001 and P010-TASK-005. Exercise positive and negative actor, action, registry and literal text-prefix filters, combinations, WorldEdit selection when installed, all supported scopes and authorization on every continuation. |
| Preview, apply, cancel, rollback, restore, undo and separately privileged recorded-item give | Phase 011. Fixed selection, safe private preview, exact applied subset, conservation and crash fixtures are mandatory. |
| Help, version, database/queue health, permissions, localized output and atomic reload | P010-TASK-005 with the existing configuration and presentation services. Test console and native chat, including denied and unavailable states. |
| Consumer pause/resume and separately authorized capture pause/resume | P010-TASK-005. Consumer pause suspends ingestion while local durable capture continues; show backlog and frozen central watermarks without inventing a gap. Only actual loss or explicitly paused capture creates a missing-history interval. Resume drains idempotently. |
| Per-world/category policies, actor/material exclusions and item-hover options | Phases 008 and 009 supply policy and capture; P010-TASK-001 proves their query visibility. Deliberate exclusions are explicit coverage states and cannot count as complete mandatory capture. |
| Manual purge and configurable scheduled retention | P010-TASK-005 and the subsection above. Default manual, confirmed activation, shared dependency guards and actual dual-engine recovery evidence. |
| Backup, integrity checking and source-preserving storage migration | Phases 008, 010 and 011, with complete integration in Phase 015. Support the owner-selected MySQL and MariaDB engines without importing unrelated upstream backends. |
| API version/enabled capability checks, typed logging, block/session/pending lookup, parsed result views, placement/removal predicates and guarded restoration/purge | P010-TASK-003 maps every pinned API method to existing typed append/query/coverage or authorized job services, with independently compiled consumer fixtures. Phase 011 supplies world-changing methods. No raw SQL, Bukkit binary types or permission bypass is exposed. |
| Pre-log extension and mod integration hooks | Phases 009 and 010. Validate immutable proposed records, bounded callbacks and typed redaction before persistence. Rejected or suppressed required coverage is visible, never silently lost. Adapter failure cannot bypass mandatory capture or invent an actor. |

No applicable row may close with documentation alone or an unavailable label used to avoid implementation. An upstream feature being paid does not make its publicly documented behavior platform-inapplicable; implement approved feasible behavior originally without copying unavailable internals. Version-inapplicable content, Bukkit binary integration, unknown private implementation and unobservable mod state require specific evidence and cannot become universal compatibility promises. Phase 015 validates the complete row set against source/catalog/API versions and each row's required real-path evidence.
...
| Pause or purge would silently erase evidence | audit.pause and purge.decision contain scope, selection digest, gaps, dependency-provider state and reason. | Consumer pause displays pending history while capture continues; explicit capture pause creates a visible missing-history interval. Manual purge requires preview/confirm. Scheduled purge requires the confirmed policy revision and frozen run selection. Both refuse unknown, unavailable or unresolved Phase 011 dependency-provider state and serialize against new job dependencies. | Expire preview without deletion; commit deletion, progress and intentional gap together only for an authorized batch; stop a schedule on revoked authorization, lost fence or disabled policy. | Preview expiry, changed selection, confirmation replay and safe synthetic provider fixture for unknown, unavailable and unresolved dependency refusal. |
...
| audit.pause, mode:enum, scope:string, first:u64?, last:u64?, pending:u64, reason:enum, resumed:bool | Audit coverage owner, one transition | Consumer pause preserves capture and exposes backlog; capture pause records the missing interval. Neither invents captured sequences or loss. |
| purge.decision, mode:enum, policyDigest:sha256?, fence:u64, selectionDigest:sha256, selected:u64, applied:u64, jobDependencies:u64, confirmation:enum, reason:enum | Audit retention controller, one activation or bounded batch decision | Manual confirmation or confirmed schedule revision, current fence and exact selection are required; unknown or unresolved dependencies refuse deletion. |
...
4. For the residual UI claim, before any natural input prepare the owned laptop client with the master Section 14 Trident instance lifecycle, verified discrete renderer, exact owned instance/runtime and Hyprland window/PID identity. Confirm node-1 dedicated-server readiness and laptop reachability within 60 seconds of client connection attempt, use a supported automatic direct-connect or authorized desktop control, and prove the intended player joined the exact server world on both sides. After client setup, start a fresh bounded audit capture and verify status so setup time cannot exhaust the capture before its stimulus. Only then perform one permitted normal inspection click and hover/native-chat detail. Do not use console setup as proof of the player permission/action.
...
| P010-TASK-005 | Inspector, exact purge selection, retention defaults and invalid reload tests. | Command/action token, consumer and capture pause, fenced scheduler and dependency admission integration. | Dedicated server natural interaction and actual dual-engine manual/scheduled deletion, including standalone operation without Velocity. | Inactive/denied click, token expiry, backlog versus gap, confirmation replay, revoked policy, lost lease, restart/crash, migration race and unknown/unavailable/unresolved dependency provider. | node-1 no-GUI dedicated server and isolated MySQL/MariaDB for state proof, with laptop evidence separately required by P010-TASK-006. | Interaction trace, before/after digest, pause states, policy/fence/selection receipts, purge boundaries and recovery evidence. |
| P010-TASK-006 | Diagnostic typed-field/limit/self-test tests. | Console enable/status/off and registered-target integration. | Laptop-only native inspection click, hover and rich-chat receipt against node-1 dedicated server. | Missing target, denied debug, output unavailable, timeout, instance replacement and client identity failure. | Split host only for presentation. Laptop requires verified desktop/discrete renderer, owned Trident instance/window binding and joined-world proof. | Sanitized JSONL/status receipts, exact client visual/input evidence, per-host cleanup receipt. |
...
Document MySQL 8.4.11, MariaDB 11.4.13 and Connector/J 3.5.10 as verification pins only. Include preflight backup, isolated empty target, source fence and spool, checkpoint/resume/rollback, source preservation, separate principal creation/revocation, TLS/trust configuration, allowLocalInfile=false, query limits, phase-available validation counts/hashes/watermarks/coverage/gaps, manual purge preview/confirm, manual-default scheduled-policy activation, typed settings and last-valid reload, fenced batch/dependency refusal, cancellation/restart recovery, disk-reclamation limits and cleanup. State that actual future restoration-job authority, backups and migration proof belong to Phases 011 and 015. Update diagnostics support steps and test-host/Trident requirements. Wiki publication waits for the approved merge. No release, production migration, remote database change, webhook, credential or public compatibility claim is authorized.
