...
- SEF-REQ-001 foundation work: create the native Forge 1.20.1 and Java 17 server-only integration boundary, retain only inventory-approved behavior, and prove dedicated standalone startup without Velocity, Ambassador, PCF, client reset or SEF client protocol/classes. The Forge loader metadata must not declare external proxy adapters as mandatory dependencies.
...
4. `P001-TASK-004` performs dependency-aware build, loader metadata/class/resource/configuration scans, dedicated standalone server readiness with proxy adapters absent, no-client requirement proof, documentation, cleanup, and phase handoff verification. Trace: SEF-PHASE-001.
...
- Dedicated no-GUI Forge server startup with `eula=true` readback and no Velocity, Ambassador, PCF or client reset installed; loader metadata and class linkage prove none is mandatory. Console diagnostics controls work, and an actual compatible client without SEF or client reset logs in for the residual client claim. The latter must use the laptop protocol below and remains open if it cannot be performed silently and with exact identity.
...
- The dedicated artifact loads without client classes/payload negotiation or mandatory external proxy adapters, starts in standalone mode with an ordinary compatible client path, and no active economy/interface surface survives. Legacy files are unchanged and inert.
...
| Server-only removal boundary | Phases 002, 003, and release integration | No SEF client protocol, custom screens/HUDs/Fancy Tags/menu utilities, economic policy, or mandatory external proxy-adapter linkage can be referenced by standalone bootstrap. | New code must be rejected by loader metadata/dependency/JAR/configuration scans if it crosses this boundary. | Static graph, JAR, loader metadata, config, command/permission, and standalone runtime startup reports. |
...
| P001-TASK-004 | SEF-REQ-001, SEF-REQ-003, SEF-REQ-004, SEF-REQ-006, SEF-REQ-019 | Execute the phase evidence ladder, package inspection, dedicated-server readiness without proxy adapters, residual compatible-client check, documentation updates, and integration handoff. Bind every result to exact candidate/source/driver/configuration/world-generation identities. | P001-TASK-001 through P001-TASK-003; checked no-GUI task graph; disposable host anchors; applicable repository checks. | Sanitized phase evidence packet, removal/migration/configuration reports, JAR manifest/loader-metadata/class/resource report, docs, cleanup receipts, PR and tag handoff material. | Forge artifact, server runtime, diagnostics output, `docs/verification/phase-001/`, `docs/test/`, `docs/migrations/`, `docs/troubleshooting/diagnostics.md`. | Formatter/static/unit/integration/build order; loader metadata has no required PCF or `clientresetpacket` entry; real dedicated server `eula=true` readiness with no Velocity/PCF/reset mod; JAR inspection; laptop-only actual compatible client login if and only if exact Trident instance/window/PID/renderer proof is available; post-merge candidate rebuild and cleanup verification. |
...
| Dedicated startup exposes a client or external proxy-adapter dependency | Gradle task graph, Forge loader metadata and JAR report identify `net.minecraft.client`, screen/HUD/payload/config-task linkage or mandatory PCF/`clientresetpacket` linkage; server startup log and diagnostic `startup.boundary` record classloader reason. | Fail standalone readiness before accepting the artifact. Do not add a required proxy or client install. | Remove/relocate the dependency and rerun standalone server startup plus relevant retained path smoke proof. | No-GUI dedicated Forge runtime reaches readiness without proxy adapters, with a compatible non-SEF client able to connect under the separate laptop gate; metadata and class/resource scans have zero prohibited linkage. |
...
| SEF-REQ-001, P001-TASK-004 | Java 17/Forge compile, forbidden client/payload linkage and final JAR inspection. | Dedicated bootstrap against merged phase candidate. | Compatible vanilla client login only for residual no-SEF-client proof; both sides confirm joined world. | Client class/payload linkage fails server gate; missing laptop identity/instance/GPU/private endpoint leaves client gate open. | Dedicated server on `node-1`; matching laptop client only after exact isolated-path, Trident instance/GPU/window/PID procedure and private endpoint readiness. | Candidate hashes, task graph, JAR report, server log, laptop evidence only if complete, per-host cleanup receipts. |
...
- Exact test-owned resource register and cleanup receipts for `node-1` and, if used, laptop: process exit, owned instance removal, disposable fixtures/logs/captures/worlds/databases/WAL files removed, required sanitized evidence retained, and any leftover named as an open gate.
