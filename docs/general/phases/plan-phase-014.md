...
5. P014-TASK-005 runs smallest silent laptop actual player switch, cross-server arrival, native chat rendering, reconnect, and residual client proof after laptop join/setup capture refresh. Trace: SEF-PHASE-014.
...
| P014-TASK-005 | SEF-REQ-034, SEF-REQ-035 | Run smallest actual silent laptop proof after the diagnostic-dependent headless proof. Use real player connection to switch from source to destination through private proxy, prove source and destination joined identity, destination arrival receipt, client synchronization, reconnect fencing, and native rich player output. | P014-TASK-003, P014-TASK-004, EXT-001, EXT-003, EXT-004, EXT-005, paired candidate/common/protocol digest. | Sanitized actual player-switch, client synchronization, receipt/render evidence or explicit open-gate record. | Laptop isolated client, private proxy endpoint, source and destination Forge backends, native chat. | Laptop only. Verify candidate parity/discrete renderer, set isolated master volume zero, bind exact window address/class/title/PID with hyprctl clients -j, mute/readback only matching PipeWire/Pulse app stream, reapply replacement streams. Wait 60 seconds proxy/private connection, 20 seconds receipt, 30 seconds shutdown. Refresh capture after laptop join/setup and reenable after restart. Stop client on identity/renderer/mute/endpoint/source/destination join failure. |
...
| Laptop mute/identity fails | renderer/window/PID/stream readback absent | Stop owned client and open only visual gate. | Continue headless proof, never global mute/node-1 graphics. | P014-TASK-005 setup record. |
...
**Client audio isolation:** Node-1 launches no client. Before launch, laptop uses an isolated pinned instance with master audio at zero-volume, discrete renderer verification, exact owned Hyprland window/PID identity, matching per-application PipeWire/Pulse mute readback, replacement stream recreation handling, and watcher/runtime teardown.
...
4. For residual gate only, launch silent isolated laptop client, join source backend through proxy, refresh capture after laptop join/setup, then cause actual player switch to destination. Wait maximum 20 seconds for destination receipt. Verify source and destination joins, client synchronization, arrival, rich rendering, reconnect behavior. Do not use console to bypass player behavior.
...
6. Stop owned client/watcher/proxy/servers/database fixtures, confirm PIDs and streams gone, remove only registered temporary runtimes/logs/captures/databases/downloads/traces/build output, verify absence, and report leftovers.
...
| SEF-REQ-034, SEF-REQ-035, P014-TASK-005 | Candidate/digest and renderer/stream identity. | Source/destination joins, actual proxy switch, client synchronization, receipt, reconnect. | Silent laptop client through real private proxy from source Forge backend to destination Forge backend with native chat. | Endpoint/mute/renderer/session/source/destination join failure stops client/open gate. | EnVy laptop isolated matched runtime, discrete renderer, per-app mute, authorized endpoint. | Sanitized client visual/log/source-destination receipt/audio teardown. |
...
| SEF-RISK-020, P014-TASK-005/006 | Pair digest, candidates, host audio/renderer, branch tags. | Digest/endpoint/identity/cleanup/tag mismatch. | Stop/block/rerun exact gate. | Any mismatched artifact/host/merge result. | Rebuild pair and repeat host proof. |
...
- Laptop packet with renderer/instance/app-stream mute, endpoint/autojoin, source and destination joins, actual player switch/client synchronization, receipt, reconnect, rich-chat residual, shutdown/stream cleanup, or exact open gate.
...
- Per-host resource/cleanup receipt covering runtimes, DB, processes, audio watcher/stream, logs, screenshots, captures, traces, downloads, generated config, bytecode, output. Exact removal after final use is verified. Cleanup defect blocks transition.
