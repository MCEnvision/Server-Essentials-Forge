...
| UNKNOWN | Runtime fidelity | No signed-chat, three-backend lifecycle, or client presentation test has run in this planning pass. | `research/brief.md` | Resolve only through the registered headless and Trident laptop evidence gates. |
...
- Compiler/property, migration, restart, crash-cut, local and one-proxy/three-backend lifecycle tests plus residual Trident laptop native-chat evidence.
...
**Client verification:** Only residual signed-chat receipt and native component/tab/entity/suggestion presentation need a matching Trident laptop client. Server or simulated-player evidence cannot close those claims. Unavailable laptop identity, renderer, connection, instance or input evidence leaves only that gate open.
...
9. Stop exact owned client, server, proxy and fixture processes after their final consumer. Confirm process exit and owned instance removal, remove only verified disposable runtimes/worlds/logs/configs/downloads/traces/reports, confirm paths absent on every used host, and record exact leftovers separately if cleanup fails.
...
|---|---|---|---|---|---|
| SEF-RISK-005, P006-TASK-001 | Backend signed-chat and supported-alias enforcement using committed replica state and absolute expiry. | Chat/command decision and independent native receipt. | Repair route, resync state, retain mute and rerun actual paths. | Any chat adapter, alias catalog, authority or pair digest change. | Unit, dedicated server and Trident client signed-chat matrix. |
...
- Sanitized node-1 server/proxy traces plus laptop native chat and visibility evidence only where client receipt/render/input is the residual claim. Record matching candidate/profile, desktop/discrete renderer, window/PID, Trident instance binding and join correlation.
