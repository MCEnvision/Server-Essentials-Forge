...
- Network feasibility and isolation. SEF-REQ-009 through SEF-REQ-018, FIND-101. Exact downloaded proxy, backend and client artifact identities are bound; the combined Forge/proxy handshake and continuous-session switching behavior still needs the early real-runtime gate. Missing client reset capability has a separate pre-move refusal gate in Phase 005. These external adapters are optional for a standalone Forge deployment, which requires separate clean-start and local-feature proof without them. Configured Ambassador network mode with an unverified SEF Velocity companion gives operators manifest-backed download or connection guidance and fences network actions; standalone mode gives no prompt.
...
Use node-1 for headless builds, servers and proxy only. DEC-019 requires Trident CLI on the verified laptop for every new client instance test, using the existing authenticated home and explicit task-owned instance. Preserve credentials, personal instances, global settings and audio. No Prism fallback, alternate launcher home, node-1 Trident command or mandatory mute gate is allowed. FIND-113 and [the Trident source record](sources/trident-testing.md) distinguish documented lifecycle behavior from runtime proof still required. Register exact disposable resources before tests and verify instance-list and filesystem cleanup after their final consumer.
