...
The latest merge-revision matrix run `33738411285` tested synthetic merge commit `646234240649a3552c50970a32b9beae9a0313a3`, which corresponds to branch head `830fb9af4cbe79694314cfebd2b6863311a1f55c`. Candidate build, Linux, macOS, and Windows runtime jobs, CI, CodeQL, and the independent review all passed. Each dedicated server reached `Done`, remained alive until the workflow sent `stop`, emitted `Stopping server`, and exited cleanly. The packaged client probe now requires the GUI atlas marker and five seconds of continued process liveness before accepting startup. Ubuntu passed startup, LWJGL, SEF initialization, and the GUI atlas. macOS was classified only by the known `NSGL` pixel-format signature, and Windows only by the known `WGL` OpenGL-driver signature. Both remain mandatory supported operating systems. The candidate artifact stayed bound to the merge commit and retained SHA-256 `da76826dd757e9f52b1cfc61fb9902812a77c328321edae14428f329c2a16eb0` and SHA-512 `ebef0a8b845576ad6a5fb635bd6ac5b79bd4de813b180ce5e2600e8397743b8871d3a5955eaa70437dbb064bd3af50e6c06904187b6556e309035b050cbafd2c`. The complete `EXT-001` failure-injection and direct graphical evidence packet remains open.
...
The merge-revision verification was rerun by `33738411285` after the final workflow hardening. Build candidate, all three operating-system runtime jobs, CI, and CodeQL completed successfully, and the independent review reported no actionable findings. The runtime evidence is tied to synthetic merge commit `646234240649a3552c50970a32b9beae9a0313a3`, not only the phase branch head. The client stability gate rejects a process that dies after printing the atlas marker, and the server gate rejects a process that reaches `Done` without staying alive for the explicit stop command. The uploaded manifests contain normalized paths only, matching artifact hashes, and no detected hosted paths or secret-like strings.
