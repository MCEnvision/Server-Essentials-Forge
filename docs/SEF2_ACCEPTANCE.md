...
- Hosted matrix run `33688452703` passed the candidate build, 530 unit tests, 41 required GameTests, native provider smoke, dependency insight, ZIP validation, and duplicate native runtime scan on Linux, macOS, and Windows. Full `EXT-001` still requires direct client and server workflow, rotation, restart, failure preservation, and fixture trace evidence.
