...
The current Phase 003 source candidate is commit `742b5ab` on `envy/phase-003-commands`. Legacy `/warn`, `/mute`, `/freeze`, `/invlock`, and `/disablebuilding` target routes now enforce shared hierarchy, exemption, vanish, and self-target policy before state mutation. Metadata-only console routes now also have live GameTest assertions for actor attribution, result mapping, redaction, bounded target and parameter projections, and audit correlation. Every intentionally unavailable control family now has live dispatcher evidence for denied, view, and manage permission projection without exposing the mutation route through view access. The catalog-wide policy regression now checks all 694 sealed actions against a denying permission provider and passes from a clean Java 21 checkout. The last artifact bound before this source-only regression is `sef-2.0.0.jar`, 3,404,210 bytes, SHA-256 `d562b7ff6441e8bfe2fd3cf1286746bf93850188040e4712ca7d30042c3861fa`; it must be rebuilt and rebound before candidate closure. The universal matrix remains intentionally incomplete with 710 rows, 694 executable rows open, 16 unavailable rows passed, and `complete=false`. Evidence is retained under `/mnt/hermes/audit-evidence/SEFPORTED/phase-003/task-075-metadata-console-audit`, `task-075-inventory`, `task-076-unavailable-permission-audit`, and `task-078-catalog-policy-denial`. This is a current Phase 003 progress record, not final acceptance.
...
- All 574 unit tests pass with zero failures and zero skipped tests.
...
- The previous Phase 003 JAR is `sef-2.0.0.jar`, 3,404,210 bytes, with SHA-256 `d562b7ff6441e8bfe2fd3cf1286746bf93850188040e4712ca7d30042c3861fa` and SHA-512 `e3f238ec9fa8044160d80726fc452918d57276ebee9ab352f7b58925a2878731822d1c8cf8607eb6d7ba95126805f765f9733505539d16440c3b31274afbe6a9`. It predates the source-only catalog policy regression at `742b5ab`, so a fresh artifact binding remains required. Hosted runtime checks passed on Linux, macOS, and Windows for the earlier bound revision.
