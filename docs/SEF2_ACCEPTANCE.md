...
- Hosted matrix run `33719642223` passed the candidate build, 530 unit tests, 41 required GameTests, native provider smoke, disposable writer probe, opened object identity traces, rejected target preservation, dependency insight, ZIP validation, duplicate native runtime scan, and packaged dedicated server workflow on Linux, macOS, and Windows. Full `EXT-001` still requires direct client workflows and the complete platform specific failure matrix.
