...
The owner resolved this evidence discrepancy in the maintained plan. The exact external Forge Client Reset Packet file `4657349` is approved for the isolated matching client profile and compatible network profile. This does not add a client module or hard dependency to SEF. The earlier fixtures did not install the reset mod, so their `Please reconnect` results are a missing-capability control, not a compatibility result for the approved profile. No reset-enabled session switch has been tested. A fresh isolated fixture must bind the exact candidate and client-mod identities, then correlate authenticated forwarding, signed chat, real command arguments, direct forwarding rejection, and A to B followed by B to A within one client session. Until those receipts pass, `SEF-AC-010` and the phase remain open.
...
## fixture 2026-09-25 k headless readiness

The `k` retry used the reobfuscated SEF JAR built from commit `e59d90c95375936349222ef7667510d029c628e4`, SHA-256 `90ff6e7f3c7ad243b119f320ac280eee64888bff5c076c50f28832e955be07ec`. The copies installed in backend A and backend B were byte-identical. The pinned Velocity, Ambassador, ProxyCompatibleForge, Forge installer, and MixinExtras candidate hashes matched their recorded identities; the client reset candidate retained its separately recorded hash and was not installed on either backend.

Both no-GUI Forge 47.3.12, Minecraft 1.20.1 backends reached `Done` and initialized SEF under Java 17.0.20.1. ProxyCompatibleForge 1.3.1 and MixinExtras Forge 0.5.3 loaded on both backends, and both EULA files were read back as `eula=true` before launch. Velocity 4.2.0 build 30 reached `Done` under Java 25.0.4.1 with Ambassador 1.4.5 loaded. The two backends bound only to loopback ports 26555 and 26556; Velocity bound only to the authorized private interface on port 26557. PCF used `MODERN_DEFAULT`; the same non-secret config digests and private fixture secret arrangement are recorded in the fixture register. The secret is not retained here.

This retry proves only pinned candidate readiness and server/proxy startup. No Minecraft client connected, so it proves no forwarded player identity, signed chat, client command arguments, reset-capability advertisement, or in-session transfer. No client was launched or interacted with during this retry. `SEF-AC-010` therefore remains open.

The exact Forge and Velocity fixture sessions were stopped after world saves. Forge process IDs 4069048 and 4070004 were absent afterward, ports 26555 through 26557 were closed, and the temporary checkout `build` output and Forge installer logs were removed after their final consumers. The registered `k` runtime tree and pinned candidate files remain only because the isolated laptop-client gate still consumes them. No task-owned client, audio stream, or audio watcher was started in this retry.
