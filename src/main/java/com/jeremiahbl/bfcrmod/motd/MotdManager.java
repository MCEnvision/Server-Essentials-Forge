...
/**
 * Manages the server MOTD. Config stored in config/bfcrr/motd.json.
 */
public class MotdManager {
    ...
    public void load() {
        ...
        try (Reader reader = Files.newBufferedReader(configPath)) {
            ...
            BetterForgeChat.LOGGER.info("[BFCRR] MOTD loaded");
        } catch (Exception e) {
            BetterForgeChat.LOGGER.error("[BFCRR] Failed to load MOTD", e);
        }
    }
    ...
    public void save() {
        try {
            ...
            BetterForgeChat.LOGGER.error("[BFCRR] Failed to save MOTD", e);
        }
    }

    public void setMotd(String line1, String line2) {
        data = new MotdData(line1, line2);
        save();
    }
    ...
    public void applyToServer(MinecraftServer server) {
        String motdStr = data.line1();
        if (data.line2() != null && !data.line2().isEmpty()) {
            motdStr += "\n" + data.line2();
        }
        if (!motdStr.isEmpty()) {
            ...
            BetterForgeChat.LOGGER.info("[BFCRR] Server MOTD applied");
        }
    }
}
