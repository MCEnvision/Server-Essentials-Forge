...
When expanded moderation is disabled and the legacy `/warn`, `/mute`, `/freeze`, `/invlock`, or `/disablebuilding` routes are registered, every target read or mutation still passes through `LegacyTargetPolicy`. The route rejects vanished targets, higher hierarchy targets, self targets where the operation is administrative, and targets holding the matching `sef.exempt.*` node unless the actor has `sef.moderation.hierarchy.bypass` or `sef.moderation.bypass.exempt`. Console remains an explicit policy bypass. A rejected target returns failure before the manager or persistent state is touched, and the same guard is used by aliases and unfreeze or unmute operations.
