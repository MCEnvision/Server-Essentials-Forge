...
When expanded moderation is disabled and the legacy `/warn`, `/mute`, `/freeze`, `/invlock`, or `/disablebuilding` routes are registered, every administrative target read or mutation passes through `LegacyTargetPolicy`. The route rejects vanished targets, higher hierarchy targets, self targets where the operation is administrative, and targets holding the matching `sef.exempt.*` node unless the actor has `sef.moderation.hierarchy.bypass` or `sef.moderation.bypass.exempt`. The explicitly self-only `/warns` read keeps its own permission path. Console remains an explicit policy bypass. A rejected target returns failure before the manager or persistent state is touched, and the same guard is used by aliases and unfreeze or unmute operations.
...
71. Every intentionally unavailable control family is checked through the live dispatcher with denied, view, and manage permission projections. View access cannot expose the mutation route, and manage access remains limited to the documented testing surface while the runtime negative contract stays fail closed.
72. Every sealed command catalog action is checked against a denying permission provider. No executable action can become usable through the shared execution boundary when authorization is denied.

The historical phase records retain the exact development commands and earlier findings. The current authoritative completion state is [the SEF 2 acceptance ledger](docs/SEF2_ACCEPTANCE.md). It records the maintained unit suite, 45 required GameTests, complete command route and parser coverage, dedicated-server and client-startup checks, migration and recovery fixtures, performance budgets, security review, JAR inspection, the dependency closure blocker, and the multiplayer and interactive gates that remain open. The twenty confirmed source defects from the full repository audit are repaired and documented in [audit.md](audit.md).
...
Server control catalog, status, feature listing, and field inspection routes enter the shared kernel execution boundary before producing output. They record bounded metadata only, preserve console actor attribution, and fail closed when mandatory audit storage is unavailable. Feature listing uses the view action when view permission is present and the manage action only for manage-only callers. Player-only metadata adapters use the same boundary for source rejection, so a console invocation receives a bounded rejected audit event instead of an unaudited early return.
